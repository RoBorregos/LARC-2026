...
// ─── Configuración ────────────────────────────────────────────
static constexpr float kSideMeters  = 1.0f;
static constexpr float kTargetRPM   = 50.0f;
...
static constexpr float kPwmDeadband = 50.0f;
...
// ─── Secuencia: FORWARD -> LEFT -> FORWARD -> LEFT ... ────────
// Solo usa direcciones donde LL va hacia ADELANTE (+RPM)
// FORWARD: todos +RPM  — LL va adelante ✅
// LEFT:    UL- UR+ LL+ LR-  — LL va adelante ✅
...
void setup() {
    ...
    Serial.println("=== FORWARD + LEFT START ===");
    Serial.println("Secuencia: FORWARD 0.5m -> LEFT 0.5m -> DONE");
    delay(2000);
...
void loop() {
    ...
    if (now - lastCycle >= (uint32_t)(kTs*1000)) {
        ...
        if (now - lastPrint >= 200) {
            ...
            Serial.print(" | tLL:");    Serial.print(ticksLL_count);
            Serial.print(" | step:");
            switch(step) {
