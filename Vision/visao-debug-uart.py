#!/usr/bin/env python3
"""
Bean Sorter — Mac UART Servo Demo Viewer   (visao-debug-uart.py)
================================================================
Same pipeline debug viewer as raspi_visao_debug.py, but tuned to run on
a Mac for a live UART demo of the servo firmware (src/main.cpp,
"servos-demo"). Camera reads 1344x376 (ZED stereo), splits into L/R
halves, downscales each 2x to 336x188.

Each frame this script sends the trigger result to the Teensy over USB
serial. The Teensy moves the two intake servos:

    LEFT  window  -> lower intake -> Teensy pin 2   (88 = bean, 55 = idle)
    RIGHT window  -> upper intake -> Teensy pin 4   (88 = bean, 55 = idle)

Just plug in the Teensy and run:

    python3 Vision/visao-debug-uart.py

The serial port is auto-detected (any /dev/tty.usbmodem* / .usbserial*),
falling back to SERIAL_PORT below if needed.

Protocol  (Mac -> Teensy):  0xFF  <left_hit>  <right_hit>

Both intakes share one resizable window (upper on top, lower on bottom).
Drag the window edge to stretch the view; the controls/sliders stay on
their own separate window.

Keys:  Q / ESC = quit   S = save config   F = fullscreen   + / - = wider / narrower
"""
...
DS = 1  # downscale factor — must match raspi_visao.py
...
#  SERIAL
# ══════════════════════════════════════════════════════════════════════
# Fallback port if auto-detect finds nothing. On a Mac the Teensy shows
# up as /dev/tty.usbmodemXXXXXXX — the number changes per board.
...
def _find_serial_port() -> str:
    """Pick the first Teensy-looking serial device, else the fallback."""
...
def _open_serial():
    ...
    try:
        _ser = serial.Serial(port, SERIAL_BAUD, timeout=0)
        print(f"[Serial] opened {port} @ {SERIAL_BAUD}")
...
def _send(left_hit: bool, right_hit: bool):
    ...
    try:
        # 0xFF header, then left (lower/pin2) and right (upper/pin4) hits
...
def main():
    ...
    print("[Keys]  Q/ESC=quit   S=save config   F=fullscreen   +/-=wider/narrower")
    ...
    # One merged, resizable window: upper intake on top, lower on bottom.
    # WINDOW_NORMAL lets the image stretch to whatever size you drag it to.
    COMBINED_WIN = "Intakes  [upper / lower]   (F=fullscreen Q=quit S=save)"
    cv2.namedWindow(COMBINED_WIN, cv2.WINDOW_NORMAL)
    cv2.moveWindow(COMBINED_WIN, 0, 30)
    cv2.moveWindow(CTRL_WIN, 1200, 30)          # controls on their own window
    stretch_x   = 1.8       # horizontal stretch of the frame itself (+/- to change)
    _fullscreen = False
    _fit_window = True       # resize the window to match the stretched frame

    # display scale uses DOWNSCALED half width for radius guide
    _scale['x'] = PANEL_W / ds_w

    cv2.setMouseCallback(COMBINED_WIN, _mouse_cb)
    ...
    while True:
        ...
        halves = [
            ('left',  cv2.resize(frame[:, :half_w], (ds_w, ds_h), interpolation=cv2.INTER_NEAREST)),  # lower intake (pin 2)
            ('right', cv2.resize(frame[:, half_w:], (ds_w, ds_h), interpolation=cv2.INTER_NEAREST)),  # upper intake (pin 4)
        ]
        ...
        for side, half in halves:
            ...
            rotated_store[side] = cv2.rotate(stacked, cv2.ROTATE_90_COUNTERCLOCKWISE)

        # upper intake (RIGHT) on top, lower intake (LEFT) on bottom
        combined = np.vstack([rotated_store['right'], rotated_store['left']])
        # actually stretch the pixels horizontally (window keeps aspect, so resize the image)
        if stretch_x != 1.0:
            combined = cv2.resize(combined, None, fx=stretch_x, fy=1.0,
                                  interpolation=cv2.INTER_LINEAR)
        cv2.imshow(COMBINED_WIN, combined)
        if _fit_window and not _fullscreen:
            cv2.resizeWindow(COMBINED_WIN, combined.shape[1], combined.shape[0])
            _fit_window = False

        left_hit  = check_trigger(cdata_store['left'],  cfg, 'left')   # lower intake (pin 2)
        right_hit = check_trigger(cdata_store['right'], cfg, 'right')  # upper intake (pin 4)
        ...
        if key == ord('f'):
            ...
            cv2.setWindowProperty(
                COMBINED_WIN, cv2.WND_PROP_FULLSCREEN,
                cv2.WINDOW_FULLSCREEN if _fullscreen else cv2.WINDOW_NORMAL)
        if key in (ord('+'), ord('=')):     # wider
            stretch_x = min(5.0, round(stretch_x + 0.2, 1))
            _fit_window = True
            print(f"[Stretch] x={stretch_x}")
        if key in (ord('-'), ord('_')):     # narrower
            stretch_x = max(0.4, round(stretch_x - 0.2, 1))
            _fit_window = True
            print(f"[Stretch] x={stretch_x}")
        if key == ord('s'):
